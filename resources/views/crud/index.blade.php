  ...
  try{
    ...
    tbl1.querySelector('tbody').innerHTML = rows.length ? rows.map((p,i)=>`
      <tr>
        ...
        <td>${esc(p.valid_until||'')}</td>
        <td>
          <button class="btn btn-warning btn-sm" onclick='spStartEdit(${p.id||i},"${esc(p.title||"")}","${esc(p.description||"")}","${esc(p.valid_until||"")}")'><i class="bi bi-pencil-square"></i></button>
  ...
  try{
    ...
    tbl2.querySelector('tbody').innerHTML = rows.length ? rows.map((u,i)=>`
      <tr>
        ...
        <td>
          <button class="btn btn-warning btn-sm" onclick='jbEdit("${u.id}","${esc(u.username||"")}","${esc(u.email||"")}")'><i class="bi bi-pencil-square"></i></button>
...
async function loadGadget(){
  try{
    ...
    tbl3.querySelector('tbody').innerHTML = rows.length ? rows.map((x,i)=>`
      <tr>
        <td>${x.id||i+1}</td><td>${esc(x.name||'')}</td><td>${esc(x.brand||'')}</td><td>${esc(x.price||'')}</td>
        <td>
          <button class="btn btn-warning btn-sm" onclick='gStart("${x.id}","${esc(x.name||"")}","${esc(x.brand||"")}","${esc(x.price||"")}")'><i class="bi bi-pencil-square"></i></button>
...
async function loadKrusit(){
  ...
  try{
    ...
    tbl4a.querySelector('tbody').innerHTML = rows.length ? rows.map(m=>`
      <tr>
        <td>${m.id}</td><td>${esc(m.name||m.nama||'')}</td><td>${esc(m.description||m.deskripsi||'')}</td><td>${esc(m.price||m.harga||'')}</td>
        <td>
          <button class="btn btn-warning btn-sm" onclick='kEdit("makanan",${m.id},"${esc(m.name||m.nama||"")}","${esc(m.description||m.deskripsi||"")}","${esc(m.price||m.harga||"")}")'><i class="bi bi-pencil-square"></i></button>
  ...
  try{
    ...
    tbl4b.querySelector('tbody').innerHTML = rows.length ? rows.map(m=>`
      <tr>
        <td>${m.id}</td><td>${esc(m.name||m.nama||'')}</td><td>${esc(m.description||m.deskripsi||'')}</td><td>${esc(m.price||m.harga||'')}</td>
        <td>
          <button class="btn btn-warning btn-sm" onclick='kEdit("minuman",${m.id},"${esc(m.name||m.nama||"")}","${esc(m.description||m.deskripsi||"")}","${esc(m.price||m.harga||"")}")'><i class="bi bi-pencil-square"></i></button>
...
async function loadCoffee(){
  ...
  try{
    ...
    tbl5a.querySelector('tbody').innerHTML = rows.length ? rows.map(m=>`
      <tr>
        <td>${m.id}</td><td>${esc(m.name||'')}</td><td>${esc(m.description||'')}</td><td>${esc(m.price||'')}</td>
        <td>
          <button class="btn btn-warning btn-sm" onclick='cEdit("kopi",${m.id},"${esc(m.name||"")}","${esc(m.description||"")}","${esc(m.price||"")}")'><i class="bi bi-pencil-square"></i></button>
  ...
  try{
    ...
    tbl5b.querySelector('tbody').innerHTML = rows.length ? rows.map(m=>`
      <tr>
        <td>${m.id}</td><td>${esc(m.name||'')}</td><td>${esc(m.description||'')}</td><td>${esc(m.price||'')}</td>
        <td>
          <button class="btn btn-warning btn-sm" onclick='cEdit("nonkopi",${m.id},"${esc(m.name||"")}","${esc(m.description||"")}","${esc(m.price||"")}")'><i class="bi bi-pencil-square"></i></button>
...
async function loadReservasi(){
  try{
    ...
    tbl6.querySelector('tbody').innerHTML = rows.length ? rows.map(x=>`
      <tr>
        <td>${x.id}</td><td>${esc(x.name||x.nama||'')}</td><td>${esc(x.phone||x.telepon||'')}</td><td>${esc(x.datetime||x.waktu||'')}</td>
        <td>
          <button class="btn btn-warning btn-sm" onclick='rEdit(${x.id},"${esc(x.name||x.nama||"")}","${esc(x.phone||x.telepon||"")}","${esc(x.datetime||x.waktu||"")}")'><i class="bi bi-pencil-square"></i></button>
...
async function loadMaguru(){
  ...
  try{
    ...
    tbl7a.querySelector('tbody').innerHTML = rows.length ? rows.map(m=>`
      <tr>
        <td>${m.id}</td><td>${esc(m.name||'')}</td><td>${esc(m.description||'')}</td><td>${esc(m.price||'')}</td>
        <td>
          <button class="btn btn-warning btn-sm" onclick='mPEdit(${m.id},"${esc(m.name||"")}","${esc(m.description||"")}","${esc(m.price||"")}")'><i class="bi bi-pencil-square"></i></button>
  ...
  try{
    ...
    tbl7b.querySelector('tbody').innerHTML = rows.length ? rows.map(x=>`
      <tr>
        <td>${x.id}</td><td>${esc(x.name||'')}</td><td>${esc(x.slug||'')}</td>
        <td>
          <button class="btn btn-warning btn-sm" onclick='mCEdit(${x.id},"${esc(x.name||"")}","${esc(x.slug||"")}")'><i class="bi bi-pencil-square"></i></button>
